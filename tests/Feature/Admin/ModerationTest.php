...
final class ModerationTest extends TestCase
{
    ...
    public function test_requires_permission_and_server_probed_duration_before_publish(): void
    {
        ...
        $this->actingAs($admin, 'admin')->withSession(['admin_mfa_verified_at' => now()->timestamp])->putJson("/api/admin/v1/reels/{$reelId}/moderation", [...$payload, 'action' => 'restore', 'reason' => 'Restored after creator appeal evidence.'])->assertOk()->assertJsonPath('data.state.state', 'published');
        $this->actingAs($user, 'sanctum')->getJson("/api/v1/reels/{$reelId}")->assertOk();
        $this->actingAs($admin, 'admin')->withSession(['admin_mfa_verified_at' => now()->timestamp])->putJson("/api/admin/v1/reels/{$reelId}/moderation", [...$payload, 'action' => 'delete', 'reason' => 'Permanent removal after repeated violations.'])->assertOk();
    ...
    public function test_admin_can_publish_a_reel_that_is_already_live(): void
    {
        $admin = Admin::create(['name' => 'Moderator', 'email' => 'mod-live@example.com', 'password' => 'password', 'status' => 'active']);
        $user = User::factory()->create();
        $creator = CreatorProfile::create(['user_id' => $user->id, 'display_name' => 'Creator']);
        $reelId = (string) Str::ulid();
        $uploadId = (string) Str::ulid();
        DB::table('media_uploads')->insert(['id' => $uploadId, 'user_id' => $user->id, 'disk' => 'local', 'path' => 'test', 'expected_mime' => 'video/mp4', 'expected_size' => 1, 'actual_size' => 1, 'state' => 'processed', 'probe' => json_encode(['duration_ms' => 12000]), 'expires_at' => now()->addHour(), 'uploaded_at' => now(), 'processed_at' => now(), 'created_at' => now(), 'updated_at' => now()]);
        DB::table('reels')->insert(['id' => $reelId, 'creator_profile_id' => $creator->id, 'state' => 'published', 'duration_ms' => 12000, 'published_at' => now(), 'created_at' => now(), 'updated_at' => now()]);
        DB::table('reel_media')->insert(['id' => (string) Str::ulid(), 'reel_id' => $reelId, 'media_upload_id' => $uploadId, 'mime' => 'video/mp4', 'duration_ms' => 12000, 'processing_state' => 'ready', 'created_at' => now(), 'updated_at' => now()]);
        $role = DB::table('roles')->insertGetId(['name' => 'moderator-live', 'created_at' => now(), 'updated_at' => now()]);
        $permission = DB::table('permissions')->where('name', 'moderation.act')->value('id') ?: DB::table('permissions')->insertGetId(['name' => 'moderation.act', 'created_at' => now(), 'updated_at' => now()]);
        DB::table('admin_role')->insert(['admin_id' => $admin->id, 'role_id' => $role]);
        DB::table('permission_role')->insert(['role_id' => $role, 'permission_id' => $permission]);
        $this->actingAs($admin, 'admin')->withSession(['admin_mfa_verified_at' => now()->timestamp])->putJson("/api/admin/v1/reels/{$reelId}/moderation", ['action' => 'publish', 'reason_code' => 'approved', 'reason' => 'Confirming the live reel remains published.'])->assertOk();
        $this->assertDatabaseHas('reels', ['id' => $reelId, 'state' => 'published']);
    }
}
