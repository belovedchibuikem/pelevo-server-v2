  ...
  return <main className="min-w-0 px-4 py-6 sm:px-7 xl:px-10">
    ...
    <div className="mx-auto max-w-5xl">
      ...
      {groups.map(group => <section key={group} className="mt-10">
        ...
        <div className="mt-4 grid gap-5">{integrations.filter(item => item.group === group).map(item => <ProviderCard key={item.provider} item={item} />)}</div>
      </section>)}
    </div>
  </main>;
}

function ProviderCard({ item }: { item: Integration }) {
  ...
  const save = async (event: FormEvent<HTMLFormElement>) => {
    ...
    const values = Object.fromEntries([...form.entries()].filter(([key]) => key !== 'reason'));
    try {
      ...
      if (response.ok) { setMessage({ ok: true, text: 'Saved. Secrets were not written to the page or logs.' }); router.reload({ only: ['integrations', 'freshAt'] }); }
  ...
  return <article className="admin-panel rounded-2xl p-6">
    ...
    <form onSubmit={save} className="mt-5 grid gap-4 sm:grid-cols-2">
      ...
      <div className="sm:col-span-2 flex flex-wrap gap-3">
        ...
        <button type="button" disabled={busy !== null} onClick={() => void test()} className="rounded-lg border border-slate-300 px-4 py-2 text-sm font-semibold disabled:opacity-50">{busy === 'test' ? 'Testing…' : 'Test connection'}</button>
      </div>
    </form>
  </article>;
}
