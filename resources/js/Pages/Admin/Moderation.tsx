...
export default function Moderation({ reels, reports, live, appeals, sanctions, brokenLinks, recentTakedowns, freshAt }: Props) {
    const decide = async (id: string, action: 'publish' | 'reject' | 'remove') => {
    ...
    return <div className="min-h-screen bg-[#090b10] text-slate-100">
        ...
        <main className="mx-auto grid max-w-7xl gap-6 px-6 py-8 md:px-10 xl:grid-cols-3">
            <Queue title="Reels awaiting action" empty="No reels require review." rows={reels} render={(row) => <article key={row.id} className="rounded-xl border border-slate-800 bg-slate-900/70 p-4"><div className="flex items-center justify-between gap-3"><code className="text-xs text-slate-400">{row.id}</code><Badge value={row.state ?? 'unknown'} /></div><p className="my-4 line-clamp-3 text-sm text-slate-200">{row.caption || 'No caption'}</p><div className="flex flex-wrap gap-2"><Button onClick={() => decide(row.id, 'publish')}>Publish</Button><Button onClick={() => decide(row.id, 'reject')}>Reject</Button><Button onClick={() => decide(row.id, 'remove')}>Remove</Button></div></article>} />
...
function Button({ children, onClick }: { children: React.ReactNode; onClick: () => void }) { return <button onClick={onClick} className="rounded-lg border border-slate-700 px-3 py-2 text-xs font-semibold text-slate-200 hover:border-teal-400 hover:text-teal-300">{children}</button>; }
